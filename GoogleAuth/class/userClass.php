...
class userClass
{
     ...
     /* Email Confirmation */
     public function userEmailConfirmation($uid)
     {
        try{
          $db = getDB();
          $stmt = $db->prepare("SELECT email_confirmation FROM users WHERE uid=:uid");  
          $stmt->bindParam("uid", $uid,PDO::PARAM_INT);
          $stmt->execute();
          $data = $stmt->fetch(PDO::FETCH_OBJ);
          return $data;
     ...
     public function userEmailConfirmationFirstLog($uid)
     {
        try{
          $db = getDB();
          $stmt = $db->prepare("Update users set email_confirmation = 0 WHERE uid=:uid");  
          $stmt->bindParam("uid", $uid,PDO::PARAM_INT);
          $stmt->execute();
          $data = $stmt->fetch(PDO::FETCH_OBJ);
          return $data;
     ...
     public function userEmailConfirmationTrue($uid)
     {
        try{
          $db = getDB();
          $stmt = $db->prepare("Update users set email_confirmation = 1 WHERE uid=:uid");  
          $stmt->bindParam("uid", $uid,PDO::PARAM_INT);
          $stmt->execute();
          $data = $stmt->fetch(PDO::FETCH_OBJ);
          return $data;
